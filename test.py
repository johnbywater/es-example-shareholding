...
class TestShareholdingApplication(TestCase):
    def setUp(self) -> None:
        self.app = ShareholdingApplication()

    def test_shareholding(self):
        # A situation...
        person = self.app.register_person(name="Joe")
        self.assertEqual(person.name, "Joe")
        company = self.app.register_company(name="Acme")
        ...
        self.assertEqual(company.count_shareholdings(person.id, share_class_name), 1)
        ...
        self.assertEqual(company.count_shareholdings(person.id, share_class_name), 2)
